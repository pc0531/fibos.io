...
| 配置名称                    | 配置含义                                        | 默认值           | 参考值 |
| --------------------------- | ----------------------------------------------- | ---------------- | ------ |
| [http](http.md)-server-address         | 本地[http](http.md)服务地址                                | 127.0.0.1:8888   |        |
| https-server-address        | 本地https服务地址                               | -                |        |
| max-body-size               | RPC请求允许最大字节                             | 1024*1024(bytes) |        |
| verbose-[http](http.md)-errors         | 显示Http返回的错误日志                          | false            |        |
| [http](http.md)-validate-host          | 验证Http请求host                                | true             |        |
...
| 配置名称               | 配置含义                                            | 默认值 | 参考值                                                       |
| ---------------------- | --------------------------------------------------- | ------ | ------------------------------------------------------------ |
| genesis-[json](json.md)           | 指定创世块数据路径                                  | -      | 文件路径 参考文件:[EOS TestNet](https://github.com/CryptoLions/EOS-Jungle-Testnet/blob/aa499583d5e7f19799d93ab569e29b39741d1bb4/genesis.json) [EOS MainNet](https://github.com/EOS-Mainnet/eos/blob/mainnet-1.1.3/mainnet-genesis.json) |
| genesis-timestamp      | 覆盖创世块中的初试时间戳                            | -      | -                                                            |
| print-genesis-[json](json.md)     | 是否打印创世数据                                    | false  | -                                                            |
...
| chain-state-[db](db.md)-size-mb       | 区块数据库中允许的最大容量                                   | 1024 (MB)   | -      |
| chain-state-[db](db.md)-guard-size-mb | 当区块数据库中剩余的数据小于此大小时可以安全地关闭节点       | 128(MB)     | -      |
| reversible-blocks-[db](db.md)-size-mb | 最大能够回滚的数据量                                         | 340(MB)     | -      |
| contracts-[console](console.md)            | 是否打印合约输出                                             | false       | -      |
...
| api                   | 请求 | 含义                     | 参数                          | 参考请求                                                     |
| --------------------- | ---- | ------------------------ | ----------------------------- | ------------------------------------------------------------ |
| get_info    | GET  | 获取与节点相关的最新信息 | -                             | curl <[http](http.md)://127.0.0.1:8888/v1/chain/get_info>               |
| get_block   | POST | 获取一个块的信息         | block_num_or_id: 区块高度或id | curl <[http](http.md)://127.0.0.1:8888/v1/chain/get_block> -X POST -d '{"block_num_or_id":1}' |
| get_account | POST | 获取账户的信息           | account_name:账户名称         | curl<[http](http.md)://127.0.0.1:8888/v1/chain/get_account> -X POST -d '{"account_name":"eosio"}' |
| get_code    | POST | 获取智能合约代码         | account_name:合约名称         | curl <[http](http.md)://127.0.0.1:8888/v1/chain/get_code> -X POST -d    |
